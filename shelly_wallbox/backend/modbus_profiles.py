...
class GenericModbusChargerProfile:
    ...
    enable_write: ModbusEnableWrite
    ...
    def read_state(
        ...
        *,
        ...
        phase_selection = cached_phase_selection
        raw_phase_selection = _optional_field_value(self.state_phase_selection, client, None)
        if raw_phase_selection is not None:
            phase_selection = normalize_phase_selection(raw_phase_selection, cached_phase_selection)
        if phase_selection not in self.supported_phase_selections:
            phase_selection = self.supported_phase_selections[0]
        return ChargerState(
...
def load_generic_modbus_charger_profile(parser: configparser.ConfigParser) -> GenericModbusChargerProfile:
    ...
    return GenericModbusChargerProfile(
        ...
        enable_write=_required_enable_write(parser),
