...
from shelly_wallbox.bootstrap.wizard_models import WizardChargerBackend, WizardProfile, WizardTransportKind
from shelly_wallbox.bootstrap.wizard_support import TRANSPORT_VALUES, default_transport_kind, host_from_input
...
def prompt_transport_inputs(
    ...
    *,
    ...
    defaults = _transport_defaults(imported, host_input)
...
def _transport_defaults(imported: ImportedWizardDefaults, host_input: str) -> tuple[str, str, str, str]:
    return (
        imported.transport_host or host_from_input(host_input),
        str(imported.transport_port or 502),
        imported.transport_device or "/dev/ttyUSB0",
        str(imported.transport_unit_id or 1),
    )
...
def non_interactive_transport_inputs(
    ...
    return (
        transport_kind,
        transport_host,
        _namespace_int(namespace, "transport_port", imported.transport_port or 502),
        transport_device,
        _namespace_int(namespace, "transport_unit_id", imported.transport_unit_id or 1),
    )
