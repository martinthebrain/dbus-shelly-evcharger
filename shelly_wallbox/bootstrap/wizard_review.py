...
from shelly_wallbox.bootstrap.wizard_support import backend_requires_transport

_ENDPOINT_PROFILES = frozenset({"native-charger", "native-charger-phase-switch", "split-topology"})
...
def _conditional_review_items(profile: str, policy_mode: str, split_preset: str | None) -> tuple[str, ...]:
    checks = (
        (profile in _ENDPOINT_PROFILES, "adapter endpoints or serial transport"),
        (profile == "native-charger-phase-switch", "phase-switch member wiring"),
        (profile == "split-topology" and split_preset in _INDIVIDUAL_ADDRESS_PRESETS, "individual meter/switch device addresses"),
        (profile == "split-topology" and split_preset in _PHASE_WIRING_PRESETS, "phase-switch member wiring"),
...
def manual_review_items(
    ...
    items.extend(_conditional_review_items(profile, policy_mode, split_preset))
    return tuple(items)
