...
TOPOLOGY_PRESET_SPECS: tuple[TopologyPresetSpec, ...] = (
    ...
    TopologyPresetSpec(
        ...
        "Tuya PM/relay measures and switches + template charger",
        ("meter", "switch", "charger"),
        "Tuya-style local HTTP adapter owns metering/switching; the template charger owns charger control",
        "This topology prepares a Tuya-compatible local HTTP bridge for metering and relay switching.",
        recommended_rank=42,
        charger_backend="template_charger",
    ),
    TopologyPresetSpec(
        "tasmota-io-template-charger",
        "Tasmota PM/relay measures and switches + template charger",
        ("meter", "switch", "charger"),
        "Tasmota HTTP adapter owns metering/switching; the template charger owns charger control",
        "This topology prepares a Tasmota HTTP device for metering and relay switching.",
        recommended_rank=44,
        charger_backend="template_charger",
    ),
    TopologyPresetSpec(
    ...
    TopologyPresetSpec(
        ...
        "Tuya PM/relay measures and switches + Modbus wallbox",
        ("meter", "switch"),
        "Tuya-style local HTTP adapter owns metering/switching; the Modbus wallbox backend owns charger control",
        "This topology prepares Tuya-style metering/switching while a Modbus wallbox owns charger control.",
        recommended_rank=52,
        charger_backend="modbus_charger",
    ),
    TopologyPresetSpec(
        "tasmota-io-modbus-charger",
        "Tasmota PM/relay measures and switches + Modbus wallbox",
        ("meter", "switch"),
        "Tasmota HTTP adapter owns metering/switching; the Modbus wallbox backend owns charger control",
        "This topology prepares Tasmota metering/switching while a Modbus wallbox owns charger control.",
        recommended_rank=54,
        charger_backend="modbus_charger",
    ),
    TopologyPresetSpec(
    ...
    TopologyPresetSpec(
        ...
        "Tuya meter + Cerbo GX Relay switch",
        ("meter",),
        "Tuya-style local HTTP meter measures energy; Cerbo GX relay backend only switches the contactor",
        "This topology prepares a Tuya-compatible meter and one local Cerbo GX relay as the switching actuator.",
        recommended_rank=25,
        uses_cerbo_relay=True,
    ),
    TopologyPresetSpec(
        "tasmota-meter-cerbo-relay",
        "Tasmota meter + Cerbo GX Relay switch",
        ("meter",),
        "Tasmota HTTP meter measures energy; Cerbo GX relay backend only switches the contactor",
        "This topology prepares a Tasmota meter and one local Cerbo GX relay as the switching actuator.",
        recommended_rank=27,
        uses_cerbo_relay=True,
    ),
    TopologyPresetSpec(
    ...
    TopologyPresetSpec(
        "tuya-meter-goe",
        "Tuya meter + native go-e charger",
        ("meter", "charger"),
        "Tuya-style local HTTP meter measures energy; go-e backend owns charger enable/current/status",
        "This topology prepares a Tuya-compatible meter plus a native go-e charger.",
        recommended_rank=45,
        charger_backend="goe_charger",
    ),
    TopologyPresetSpec(
        "tasmota-meter-goe",
        "Tasmota meter + native go-e charger",
        ("meter", "charger"),
        "Tasmota HTTP meter measures energy; go-e backend owns charger enable/current/status",
        "This topology prepares a Tasmota meter plus a native go-e charger.",
        recommended_rank=47,
        charger_backend="goe_charger",
    ),
    TopologyPresetSpec(
    ...
    TopologyPresetSpec(
        ...
        "Tuya meter + native Modbus wallbox",
        ("meter", "charger"),
        "Tuya-style local HTTP meter measures energy; the Modbus wallbox backend owns charger enable/current/status",
        "This topology prepares a Tuya-compatible meter plus a native Modbus wallbox backend.",
        recommended_rank=55,
        charger_backend="modbus_charger",
    ),
    TopologyPresetSpec(
        "tasmota-meter-modbus-charger",
        "Tasmota meter + native Modbus wallbox",
        ("meter", "charger"),
        "Tasmota HTTP meter measures energy; the Modbus wallbox backend owns charger enable/current/status",
        "This topology prepares a Tasmota meter plus a native Modbus wallbox backend.",
        recommended_rank=57,
        charger_backend="modbus_charger",
    ),
    TopologyPresetSpec(
