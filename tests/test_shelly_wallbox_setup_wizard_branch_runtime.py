...
from shelly_wallbox.bootstrap import wizard, wizard_cli
from shelly_wallbox.bootstrap.wizard_guidance import prompt_split_preset, resolved_primary_host
from shelly_wallbox.bootstrap.wizard_import import (
...
from shelly_wallbox.bootstrap.wizard_models import WizardResult
from shelly_wallbox.bootstrap.wizard_persistence import _topology_summary_text
from shelly_wallbox.bootstrap.wizard_policy_guidance import prompt_policy_defaults
from shelly_wallbox.bootstrap.wizard_transport_guidance import preset_specific_defaults
from shelly_wallbox.core.common_schedule import scheduled_mode_snapshot
...
class TestShellyWallboxWizardBranchRuntime(unittest.TestCase):
    def test_wizard_core_helpers_cover_live_checks_write_paths_and_main_guard(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch("shelly_wallbox.bootstrap.wizard.load_backend_selection", return_value=selection),
                patch("shelly_wallbox.bootstrap.wizard.probe_meter_backend", return_value={"type": "meter"}) as meter_probe,
                patch("shelly_wallbox.bootstrap.wizard.read_charger_backend", side_effect=RuntimeError("boom")),
            ):
            ...
            with (
                patch("shelly_wallbox.bootstrap.wizard.load_backend_selection", return_value=selection),
                patch("shelly_wallbox.bootstrap.wizard.probe_meter_backend", return_value={"type": "meter"}),
                patch("shelly_wallbox.bootstrap.wizard.probe_switch_backend", return_value={"type": "switch"}),
                patch("shelly_wallbox.bootstrap.wizard.read_charger_backend", return_value={"type": "charger"}),
            ):
            ...
            with patch(
                "shelly_wallbox.bootstrap.wizard._live_connectivity_payload",
            ...
            preview = _result()
            with patch("shelly_wallbox.bootstrap.wizard._interactive_write_confirmed", return_value=False):
                with self.assertRaisesRegex(ValueError, "cancelled"):
                    wizard._confirm_write(_namespace(), preview, ("existing.ini",))
            with patch("shelly_wallbox.bootstrap.wizard._interactive_write_confirmed", return_value=True):
                wizard._confirm_write(_namespace(), preview, ("existing.ini",))
            with (
                patch("shelly_wallbox.bootstrap.wizard.prompt_yes_no", return_value=True),
                redirect_stdout(io.StringIO()),
            ):
                self.assertTrue(wizard._interactive_write_confirmed(preview, ("existing.ini",)))

            with patch("shelly_wallbox.bootstrap.wizard.prompt_yes_no", return_value=True):
        ...
        with redirect_stdout(stdout):
            with self.assertRaises(SystemExit) as raised:
                with patch("sys.argv", ["wizard.py", "--non-interactive", "--dry-run", "--json", "--profile", "simple-relay", "--host", "192.168.1.44"]):
                    runpy.run_module("shelly_wallbox.bootstrap.wizard", run_name="__main__")
    ...
    def test_wizard_cli_helpers_cover_prompt_loops_imports_and_noninteractive_errors(self) -> None:
        ...
        with patch("shelly_wallbox.bootstrap.wizard_cli.prompt_yes_no", return_value=True):
            self.assertEqual(wizard_cli._prompt_password("imported-secret"), "imported-secret")
        with patch("shelly_wallbox.bootstrap.wizard_cli.getpass.getpass", return_value="typed-secret"):
        ...
        with patch("shelly_wallbox.bootstrap.wizard_cli._prompt_choice", return_value="goe_charger"):
            self.assertEqual(wizard_cli._interactive_backend_choice("native-charger", None), "goe_charger")
        with patch("shelly_wallbox.bootstrap.wizard_cli._prompt_choice", return_value="simpleevse_charger"):
        ...
        self.assertEqual(
            wizard_cli._interactive_transport_inputs("goe_charger", "goe.local", imported),
            ("serial_rtu", "goe.local", 502, "/dev/ttyUSB0", 1),
        )
        with patch("shelly_wallbox.bootstrap.wizard_cli.prompt_transport_inputs", return_value=("tcp", "modbus.local", 1502, "/dev/ttyUSB0", 3)):
            self.assertEqual(
                wizard_cli._interactive_transport_inputs("modbus_charger", "modbus.local", imported),
        ...
        with patch("shelly_wallbox.bootstrap.wizard_cli.prompt_split_preset", return_value="template-stack"):
            self.assertEqual(
        ...
        with patch("shelly_wallbox.bootstrap.wizard_cli._prompt_text", return_value="77"):
            self.assertEqual(wizard_cli._interactive_device_instance(_namespace(), imported), 77)

        stdout = io.StringIO()
        with (
            patch("shelly_wallbox.bootstrap.wizard_cli._interactive_profile", return_value="split-topology"),
            patch("shelly_wallbox.bootstrap.wizard_cli._prompt_text", side_effect=["shared.local", "81"]),
            patch("shelly_wallbox.bootstrap.wizard_cli._interactive_split_preset", return_value="template-stack"),
            patch("shelly_wallbox.bootstrap.wizard_cli._interactive_backend", return_value="template_charger"),
            patch("shelly_wallbox.bootstrap.wizard_cli.prompt_role_hosts", return_value=("meter.local", "switch.local", "charger.local")),
            patch("shelly_wallbox.bootstrap.wizard_cli._interactive_transport_inputs", return_value=("serial_rtu", "shared.local", 502, "/dev/ttyUSB0", 1)),
            patch("shelly_wallbox.bootstrap.wizard_cli._interactive_auth_inputs", return_value=(False, "", "")),
            patch("shelly_wallbox.bootstrap.wizard_cli._interactive_policy_mode", return_value="manual"),
            patch("shelly_wallbox.bootstrap.wizard_cli.prompt_preset_specific_defaults", return_value=(None, "P1,P1_P2,P1_P2_P3")),
            patch("shelly_wallbox.bootstrap.wizard_cli.prompt_policy_defaults", return_value=(None, None, None, None, None, None, None)),
            patch("shelly_wallbox.bootstrap.wizard_cli.role_prompt_intro", return_value="Intro text"),
